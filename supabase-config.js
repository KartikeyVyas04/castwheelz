...
function renderNavbar() {
    ...
    // Advanced Navbar HTML & Injected CSS
    ...
    <style>
        ...
        @media (max-width: 768px) { .cwz-nav { padding: 15px 20px; flex-direction: column; gap: 20px; } .cwz-links { gap: 15px; flex-wrap: wrap; justify-content: center; } }
    </style>
    <nav class="cwz-nav">
...
async function checkAuthState() {
    ...
    if (session) {
        if (authLink) {
            authLink.innerHTML = `<span style="color: var(--bone); font-family: 'JetBrains Mono'; font-size: 11px; text-transform: uppercase;">${session.user.user_metadata.full_name}</span> <a href="#" onclick="signOut()" style="margin-left: 8px; color: var(--magenta); text-decoration: none; font-size: 10px; letter-spacing: 1px;">[EXIT]</a>`;
        }
        return session.user;
    } else {
        if (authLink) {
